...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ...
  app.get("/api/play-store", async (req, res) => {
    ...
    try {
      ...
      const downloadsMatch = html.match(/<div[^>]*class="ClgA3b"[^>]*>([^<]+)<\/div>/i) || html.match(/"Downloads"[^,]*"[^"]*([^"]+)"/i) || html.match(/&quot;Downloads&quot;,null,&quot;([^&]+)/i);
      if (downloadsMatch) {
      ...
      const iconMatch = html.match(/<meta[^>]*property="og:image"[^>]*content="([^"]+)"/i) || html.match(/<img[^>]*src="([^"]+)"[^>]*itemprop="image"/i);
      if (iconMatch) {
        iconUrl = iconMatch[1];
      }
      if (!rating || !downloads) {
  ...
  app.post("/api/contact", async (req, res) => {
    ...
    const ticketId = `TICKET-CDGC-363001-${Math.floor(1e5 + Math.random() * 9e5)}`;
    ...
    try {
      ...
      const mailOptions = {
        from: `"${name}" <${smtpUser}>`,
        // SMTP mail sender should be authorized user to bypass spam filters
        replyTo: email,
        to: smtpToEmail,
